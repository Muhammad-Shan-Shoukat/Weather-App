...
//         <li >
//          { <img src={`https://openweathermap.org/img/wn/${item.Icon}@2x.png`} className="relative left-20 w-[120px] h-[120px]" />
//          }
//       </li>
...
const WeatherApp = () => {
  ...
  const ShowCityData = () => {
    const API = `https://api.openweathermap.org/data/2.5/weather?q=${GetCity}&appid=383f2bad0a724304284af8932de2aab0&units=metric`;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-900 to-black text-white flex flex-col items-center px-4">
      ...
      {/* Input Section */}
      <div className="flex w-full max-w-md mt-6 gap-2">
        <input
          type="text"
          placeholder="Enter city name"
          value={GetCity}
          onChange={(e) => SetCity(e.target.value)}
          onKeyDown={(e) => e.key === "Enter" && ShowCityData()}
          className="flex-1 h-10 px-3 rounded-lg bg-white/20 text-white placeholder-white/70 outline-none"
        />

        <button
          onClick={ShowCityData}
          className="w-12 h-10 flex items-center justify-center bg-white/30 rounded-lg hover:bg-white/40"
        >
          <Search size={18} />
        </button>
      </div>
      ...
      <div className="w-full max-w-md mt-10">
        {GetWeatherData.map((item, index) => (
          ...
          >
            ...
            <h2 className="text-5xl font-bold mt-2">{item.Temp}°C</h2>
